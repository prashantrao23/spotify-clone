            ...
            <div className=''>
                <div className={`shadow-lg shadow-black/50 relative ${artist ? 'rounded-full' : 'rounded-md'}`}>
                    {image.map((image, index) => (
                        <img src={image.url} alt={image.url} key={index} className={`${artist ? 'rounded-full' : 'rounded-md'} mb-4 object-contain`} />
                    )).slice(-2)[0]}
                ...
                <div className='w-full overflow-hidden'>
                    <p className='font-bold overflow-ellipsis whitespace-nowrap overflow-hidden '>{item}</p>
                </div>
            </div>
        </div>
    )
